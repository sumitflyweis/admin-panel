...
module.exports.addhoroscope = async (req, res) => {
  try {
    const { date, horoScope, profession, emotion, health, travel, love, luck } =
      req.body;
    if (
      !(
        date &&
      ...
      let getDetails = await HoroSchema.create({
        date,
        horoScope,
        profession,
        emotion,
...
module.exports.updatehoroscope = async (req, res) => {
  ...
  try {
    // const date =new Date();
    const data = await HoroSchema.findByIdAndUpdate(id, {
      date,
      horoScope,
      profession,
      emotion,
...
module.exports.deleteHoroscope = async (req, res) => {
  const id = req.params.id;
  try {
    const data = await HoroSchema.findByIdAndDelete({ _id: id });
